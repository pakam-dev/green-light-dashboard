...
import { generateWasteByLocation, generatePickupsByLocation } from "@/lib/reportsMockData";
...
export const LocationsTab = ({ filters }: LocationsTabProps) => {
  const wasteData   = useMemo(() => generateWasteByLocation(filters.from, filters.to),   [filters.from, filters.to]);
  const pickupsData = useMemo(() => generatePickupsByLocation(filters.from, filters.to), [filters.from, filters.to]);
  ...
  const bcRate = bestCompletion
    ? ((bestCompletion.completed / (bestCompletion.completed + bestCompletion.missed)) * 100).toFixed(1)
    : "0";
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl border border-border p-5 shadow-sm flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-foreground mt-0.5">{top?.location}</p>
        ...
        <div className="bg-white rounded-2xl border border-border p-5 shadow-sm flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-foreground mt-0.5">{bestCompletion?.location}</p>
            <p className="text-xs text-muted-foreground mt-0.5">{bcRate}% completion</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-border p-5 shadow-sm flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-foreground mt-0.5">{bottom?.location}</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {bottom ? `${(bottom.wasteKg / 1000).toFixed(1)}k kg — lowest volume` : "—"}
            </p>
          </div>
        </div>
      </div>
